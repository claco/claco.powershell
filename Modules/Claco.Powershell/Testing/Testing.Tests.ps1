...
Describe 'Assert-ContainsStackTrace' {
    It "Succeeds when message contains 'Exception StackTrace' and function 'TestException'" {
        (Assert-ContainsStackTrace $StackTraceOutput).Succeeded | Should -BeTrue
    }

    It "Succeeds when message contains 'Exception StackTrace' and -WithFunctionName" {
        (Assert-ContainsStackTrace $StackTraceOutput -WithFunctionName Assert-ContainsStackTrace).Succeeded | Should -BeTrue
    }

    It 'Supports -Negate to verify message does not contain stack trace' {
        (Assert-ContainsStackTrace $StackTraceOutput -WithFunctionName Invoke-NonExistingFunction -Negate).Succeeded | Should -BeTrue
    }

    It 'Accepts scalar input in addition to array input' {
        $ScalarInput = $StackTraceOutput | Out-String
        (Assert-ContainsStackTrace $ScalarInput).Succeeded | Should -BeTrue
    }

    It 'Fails when message does not contain stack trace' {
        $AssertResult = Assert-ContainsStackTrace $StackTraceOutput -WithFunctionName Invoke-NonExistingFunction
