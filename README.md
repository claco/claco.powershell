...
foreach ($ProcessingPath in $Path) {
  ...
  if ($Content -like 'REMOVEME') {
    if ($PSCmdlet.ShouldProcess($ProcessingPath, 'REMOVE')) {
...
A more detailed breakdown of the module code itself is located in [Module Detais](#module-details) information.
...
This has the added benefit that you are testing your `.psm1` memoer exports, and not relying on any accidental global loading of the entire module under test.
...
By default, exceptions are caught in `catch` blocks, and `Write-Exception` write the error message and stack trace to the `Warning` stream using `Write-Warning`.

```powershell
Invoke-SimpleExample
WARNING: ┈┈ Exception StackTrace ┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
WARNING: The term 'Write-Deug' is not recognized as a name of a cmdlet, function, script file, or executable program.
...
Invoke-SimpleExample
Write-Error: The term 'Write-Deug' is not recognized as a name of a cmdlet, function, script file, or executable program. Check the spelling of the name, or if a path was included, verify that the path is correct and try again.
```
